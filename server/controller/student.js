...
module.exports.processTestYourselfCustomization = (req, res, next) => {
  ...
  if (req.body.part) {
    if (req.body.part.length == 1) {
      StudentPrompts.find({isTask1: req.body.part == "1", isAcademic: req.body.type === "academic"}, (err, prompts) => { 
        if(err){
          console.log(err);
          res.end(err);
        }
      ...
      StudentPrompts.find({isAcademic: req.body.type === "academic"}, (err, prompts) => { 
        if(err){
          console.log(err);
          res.end(err);
        }
...
module.exports.submitEssays = (req, res, next) => {
  ...
  Essay.create(essay1,(err, data) => {
    if(err){
      console.log(err);
      res.end(err);
    }
  });
  Essay.create(essay2,(err, data) => {
    if(err){
      console.log(err);
      res.end(err);
    }
